...
//복습) 함수, 매개변수, 리턴
//더하기, 빼기 계산기
function calcFunc(n1, n2, oper='+'){//함수만으로는 절대 실행안됨. 밖에서 함수를 실행시켜야 함.
    let num1 = n1;
    let num2 = n2;
    return oper == '+' ? num1+num2 : num1-num2; //1번은 대입, 2번은 일치여부
    //조건? 참 : 거짓;  3항 조건 연산자라 부름,
    let total = num1 + num2;
    return total; //외부로 반환
}
...
function discountFunc(price){
    ...
    return `사용자 입력가격 : ${price.toLocaleString('ko-kr')}원 기준
            ...
            30% 할인가 : ${discount30}원,
            40% 할인가 : ${discount40}원,
            50% 할인가 : ${discount50}원`//벡트기호로 묶음 템플릿
}
console.log(discountFunc(54000));
console.log(discountFunc(29900));
console.log(discountFunc(15000030));
...
//2. 콜백함수(함수 안 또다른 함수 호출) :목적에 따른 역할분담
...
orderCoffee(sns); // sns라는 조건이 있으면 orderCoffee가 실행됨을 부르는 함수
...
plusBtn.addEventListener('click',()=>{
    ...
    //재고수량10개
    currentNum > 9 ? alert('최대 구매 수량입니다. 더이상 구매할 수 없습니다.') : (()=>{
