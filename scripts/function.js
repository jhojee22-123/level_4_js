...
//v2. 이벤트함수addEventListner 내에서 직접 사용자 함수를 만드는 방식(반복안될때)
...
//변수생성 = 함수1(함수2()); //함수2부터 처리->함수1 처리 -> 변수대입
...
test();
//console.log(a); //error <위에서 내부에서만 쓰기로 설정한거니까 안됨..
console.log(b+c); //ok
...
story2(undefined, '금'); //앞의 매개변수 정의안돼서 기본값, 뒤 문자만 매개변수 대입
story2('주인공'); //앞 매개변수 문자대입, 뒤 매개변수 안적어서 기본값 처리

//카페 키오스크
//아이스아메리카노 1잔 주문완료
//function 간편버전 => 이벤트 내에서 주 사용
//''+변수+'' 간편버전 `문자${변수}` 탬플릿문자열
...
cafeKiosk_v2(0,0,1);//ice 아메리카노 1잔
cafeKiosk_v2(1,2,3);//hot 녹차라떼 3잔
...
//함수 내부 리턴에서 출력함수가 정해지지 않고 데이터만 반환했을 때 함수밖에서 출력함수를 정하는 방식
